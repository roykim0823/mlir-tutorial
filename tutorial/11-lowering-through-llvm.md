...
This is the series' payoff tutorial: a program written in our own `poly`
dialect becomes a **native executable**, called from ordinary C code, and
prints the right answer. Tutorial 10 took one step down the staircase
(`poly` → `tensor`/`scf`/`arith`); this tutorial takes all the rest — and
...
**Prerequisites:** [Tutorial 10](10-dialect-conversion.md), plus Tutorial
...
tutorial uses `mlir-translate`, `llc`, and `clang` — all in the same
`bin/` directories as Tutorial 2 §2's tools.

---

## 1. Concepts: pipelines and the exit dialect

Tutorial 2 §8 lowered `math.ctlz` to executable form with a five-pass
`-pass-pipeline` string. For `poly` the journey is ~18 passes, and typing
...
a single `--poly-to-llvm` flag. The pipeline's destination is the `llvm`
dialect — the **exit dialect**: a faithful MLIR mirror of LLVM IR, from
...
How do you *compose* such a pipeline? The article is refreshingly honest,
and its method is worth quoting as method: start from an empty pipeline;
look at the highest-level op remaining in your IR; add the pass that
lowers it; when that pass complains, figure out what it needed *first*;
...
void polyToLLVMPipelineBuilder(mlir::OpPassManager &manager) {
  ...
  // Stage 5: the LLVM descent (Tutorial 2 §8's staircase, extended)
  ...
  // Cleanup (all Tutorial 7/6 acquaintances)
  manager.addPass(mlir::createCanonicalizerPass());
...
Stage 1 you own (Tutorial 10). The cleanup quartet you know (canonicalize
and sccp are Tutorial 7; cse is Tutorial 6; symbol-dce deletes now-unused
...
After Tutorial 10 our add is `arith.addi` *on tensors* — and here's the
snag: `convert-arith-to-llvm` only handles scalar arith. LLVM has no
tensors; someone must decide what a tensor `addi` *means* in terms of
loops and memory. That someone is **linalg**, MLIR's structured
linear-algebra dialect. `convert-elementwise-to-linalg` (legal because of
`ElementwiseMappable` — Tutorial 6 paying rent again) rewrites the tensor
...
`convert-tensor-to-linalg` does the same for `tensor.pad` (Tutorial 10's
from_tensor lowering), which becomes a `linalg.fill` plus insertion.

### Stage 3: bufferization — values become memory

Everything so far — SSA values, `tensor`s, Tutorial 10's "functional
...
buffer — Tutorial 3 §6 introduced memref), and ops that *produced* new
...
- `bufferizeFunctionBoundaries = true` converted the *signature* too
  ...
  signatures" issue as Tutorial 10 §4, handled by an option here. The
...
- The `linalg.generic` now writes its result **into `%arg0`** — the
  ...
  analysis — its predecessor was a fleet of per-dialect passes, which is
  why the name celebrates being *one* pass.
- Where buffers *are* allocated, someone must free them:
  `buildBufferDeallocationPipeline` appends the official
  ownership-analysis passes that insert `dealloc`s — compiler-generated
  memory management.

### Stages 4–5: down the familiar staircase

With buffer semantics, `convert-linalg-to-loops` turns each
`linalg.generic` into `scf.for` loops over memrefs (it *requires*
bufferized input — see section 6's war story). From here it's Tutorial 2
§8 verbatim, extended for memory: `scf` → `cf` → `llvm`; scalar arith →
`llvm`; `func` → `llvm.func`; `finalize-memref-to-llvm` lowers memref ops
to pointer arithmetic (and the `index` type to `i64`); and
`reconcile-unrealized-casts` sweeps the temporary casts — Tutorial 10 §6
taught you exactly what those are. `expand-strided-metadata`, the one
...
custom op 'bufferization.clone'` (verified). The pipeline, like the
article, is honest tutorial code: scalar-out functions work end to end;
...
with the LLVM tools on `$PATH` per Tutorial 2 §2):
...
(`llvm` dialect → real LLVM IR text); past it, MLIR is out of the
...
Tutorial 10's Horner `scf.for` is now branches and **phi nodes** — where
MLIR used block arguments (`^bb3(%7: i64, ...)`, Tutorial 2 §8), LLVM
...
Every lit feature here is from Tutorial 2 — `%t`, chained RUN lines,
FileCheck on the *runtime* output — plus one: `%project_source_dir`, the
repo-defined substitution (declared in both `lit.cfg.py` and
`lit.cmake.cfg.py`, as Tutorial 2 §6 mentioned) that locates the `.c`
file from the test's sandbox. This is the fully-armed version of
Tutorial 2 §8's functional testing: not an interpreter this time, but an
...
The article documents its dead ends, and they're the most reusable part:
...
- **A plague of `unrealized_conversion_cast i64 → index`.** Cured by
  ...
  `reconcile-unrealized-casts` — Tutorial 2's mysterious final pass, now
  fully demystified.
- **The best one: an off-by-one caught by running the program.** The
  ...
  That is the whole argument of Tutorial 2 §8 (and Tutorial 10's
  exercise 4), delivered by the series itself: lowerings lie until you
  execute them.

## Differences from the original article

- The repo's pipeline targets the pinned submodule LLVM: it uses
  `OneShotBufferizePassOptions` and `createSCFToControlFlowPass`, which
  on LLVM 20 (e.g. Homebrew) are `OneShotBufferizationOptions` /
  `createConvertSCFToCFPass`. (This tutorial's outputs were produced with
  a driver using the LLVM-20 spellings of the same passes — that rename
  is also why `tutorial-opt.cpp` is the one file that won't compile
  against Homebrew LLVM, as noted since Tutorial 2's CMake caveats.)
- The buffer-deallocation step is the packaged
  `buildBufferDeallocationPipeline` rather than the article's hand-listed
  five passes — same passes, official bundle.
- The article's test used a hardcoded `llc` without
  `--relocation-model=pic`; current toolchains on macOS/arm64 want PIC,
  and the repo's RUN line has it.

## Where to go next

The `poly` story is complete: from custom syntax (Tutorial 5) to a
number printed by a C program. The final full tutorial changes
...
[Tutorial 12: A Global Optimization and Dataflow Analysis](12-global-optimization-and-dataflow-analysis.md)
...
3. Adapt Tutorial 2 §8's approach: instead of `llc`+`clang`, feed the
