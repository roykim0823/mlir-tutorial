# Chapter 9 code: the C++ pattern, before DRR

[`DifferenceOfSquares.cpp`](DifferenceOfSquares.cpp) contains the C++
`DifferenceOfSquares` rewrite pattern exactly as
[Chapter 9 §3](../../09-canonicalizers-and-drr.md) presents it — the
original version, which the repo has since replaced with the DRR
pattern in `lib/Dialect/Poly/PolyPatterns.td` (Chapter 9 §4).

Two pieces of scaffolding around the pattern are **not** part of the
original:

- The pattern was originally registered in
  ...
  dialect in `lib/`.
- One line differs from the original listing: `replaceOp(op, {newMul})`
...
pattern matches — Chapter 3 §7's "AndFold" behavior.
